...
for($i=0;$i<count($expc);$i++){
  $subid = $expc[$i];
  $res = mysqli_query("select ItemID,SubItemName,Type,Price,Quantity from subitems where SubItemID='$subid'");
  $row = mysqli_fetch_array($res);
  $itemId = $row['ItemID'];
  $res1 = mysqli_query("select count(*) as ItemCount from subitems where ItemID = $itemId");
  ...
  $res = mysqli_query("select ItemName from items where ItemID='$itemId'");
  $row = mysqli_fetch_array($res);
  ...
  $res = mysqli_query("update subitems set Popularity = Popularity*$popFactor where SubItemID='$subid'");
